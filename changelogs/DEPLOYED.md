...
| v3.18.4 | — | *not deployed* | 🔴 08-03 auto-run fixes — the `/activity-logs/?user=` author filter was a Kai disclosure the redaction could not cover; v3.18.3's second axis only fired when both identity flags were held; shared case resolution (`rows_for_cases_q`); CSV export bounded; Kai flags memoised per request; calendar upcoming-events bounded. **No migration.** ⚠️ **15 new tests have never been executed — run `manage.py test src.test_kai_audit_log src.test_kai_party_safe_surfaces` before deploying.** |
