...
Each link mirrors its dashboard's own access predicate exactly (see the
comments in `src/view/home.py`), so these tests exercise the same
predicates from the outside: someone WITH access sees the link, someone
WITHOUT it does not. Slating already has its own equivalent
(`has_slating_access`) and isn't covered here.
"""
...
class EducationDashboardLinkTests(TestCase):
    ...
    def test_officer_sees_the_link_without_being_a_chair(self):
        links = committee_dashboard_links(self.officer)
        self.assertTrue(any(l['label'] == 'Education' for l in links))
