...
class CsrfFailureRequestShapeTests(TestCase):
    """
    v3.29.29, second addendum — TEMPORARY. The 09-08 log proved a
    `source=about_to_submit` beacon can fire (code reached the line
    immediately before `form.submit()` with a real token and the marker
    both already set) and the very next failure STILL carried neither —
    meaning that failure can't be the request THIS call produced; it has
    to be a second, different submission. These fields test whether that
    second submission even resembles the real upload (a multipart body,
    the file field, several other fields) or something much thinner.
    Remove alongside the rest of this temporary logging once the root
    ...
    def test_content_length_and_type_are_logged(self):
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y'})
        line = cm.output[0]
        self.assertIn('content_length=', line)
        self.assertIn('content_type=', line)
        # A urlencoded POST body is never empty/absent here — two fields
        # were submitted, so this must NOT read as the "no body" dash.
        self.assertNotIn('content_length=-', line)

    def test_post_field_count_reflects_the_submitted_body(self):
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y'})
        line = cm.output[0]
        # username + password — no csrfmiddlewaretoken was submitted at
        # all in this failure (that's WHY it's failing).
        self.assertIn('post_field_count=2', line)

    def test_has_file_is_true_when_the_body_carries_one(self):
        upload = SimpleUploadedFile('doc.txt', b'diagnostic test content', content_type='text/plain')
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y', 'file': upload})
        line = cm.output[0]
        self.assertIn('has_file=True', line)

    def test_has_file_is_false_when_the_body_carries_no_file(self):
        """Control — same request shape as every other test in this file."""
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y'})
        line = cm.output[0]
        self.assertIn('has_file=False', line)

    def test_a_larger_body_produces_a_larger_content_length(self):
        """
        Not a fixed-number assertion (content_length reflects real
        multipart encoding overhead, which varies) — just confirms the
        field actually tracks body size rather than being a static/wrong
        value, so a real repro's number is trustworthy to compare against
        what a real upload should weigh.
        """
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y'})
        small_line = cm.output[0]
        small_length = int(small_line.split('content_length=')[1].split(' ')[0])

        upload = SimpleUploadedFile('doc.txt', b'x' * 5000, content_type='text/plain')
        with self.assertLogs('security', level='WARNING') as cm:
            self.client.post(reverse('login'), {'username': 'x', 'password': 'y', 'file': upload})
        large_line = cm.output[0]
        large_length = int(large_line.split('content_length=')[1].split(' ')[0])

        self.assertGreater(large_length, small_length)
